module ModalHelper
  ...
  def close_button(dismiss='modal')
    #It doesn't seem to like content_tag, so we do this instead.
    raw("<button type=\"button\" class=\"close\" data-dismiss=\"#{dismiss}\" aria-hidden=\"true\">&times;</button>\n")
  end
  ...
  def modal_cancel_button(content, options = {})
    ...
    content_tag :button, content, default_options.merge(options)
  end
